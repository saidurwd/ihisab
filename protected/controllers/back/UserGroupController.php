...
class UserGroupController extends BackEndController {
    ...
    public function actionDelete($id) {
        $val = $this->checkExist($id);
        try {
            if ($val > 0) {
                if (!isset($_GET['ajax'])) {
                    Yii::app()->user->setFlash('success', 'Deleted Unsuccessful. Data exist!');
                } else {
                    echo "<div class='flash-success'>Deleted Unsuccessful.  Data exist!</div>";
                }
            } else {
            ...
            if (!isset($_GET['ajax'])) {
                ...
                echo "<div class='flash-error'>Invalid request. Please do not repeat this request again.</div>"; //for ajax
            }
        }
    ...
    public function checkExist($id) {
        $val = Yii::app()->db->createCommand()
                ->select('COUNT(*)')
                ->from('{{user}}')
                ->where('group_id=:group_id', array(':group_id' => $id))
                ->queryScalar();

        $val_admin = Yii::app()->db->createCommand()
                ->select('COUNT(*)')
                ->from('{{user_admin}}')
                ->where('group_id=:group_id', array(':group_id' => $id))
                ->queryScalar();
        $totalcount = $val_admin + $val;
        return $totalcount;
    }
