...
class WhFontSizes extends CInputWidget
{
	/**
	...
	public function renderField()
	{
		...
		if ($this->useHelperSelectBox) {
			...
			$this->htmlOptions['data-size'] = $this->hasModel()
	...
	public function registerClientScript()
	{
		...
		$cs->registerScriptFile($assetsUrl . '/js/bootstrap-formhelpers-fontsizes.codes.js');
		$cs->registerScriptFile($assetsUrl . '/js/bootstrap-formhelpers-fontsizes.js');

		/* initialize plugin */
		if (!$this->useHelperSelectBox) {
			$selector = '#' . TbArray::getValue('id', $this->htmlOptions, $this->getId());
			$this->getApi()->registerPlugin('bfhfontsizes', $selector, $this->pluginOptions);
		}
	}
}